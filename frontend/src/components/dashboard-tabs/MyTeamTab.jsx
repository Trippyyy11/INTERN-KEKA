...
const MyTeamTab = ({
    ...
    const bentoPanelStyle = {
        background: isLightMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(15, 23, 42, 0.4)',
        backdropFilter: 'blur(20px)',
        borderRadius: '28px',
        border: `1px solid ${isLightMode ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.06)'}`,
        padding: '1.5rem',
        boxShadow: isLightMode ? '0 10px 30px rgba(0,0,0,0.04)' : '0 10px 40px rgba(0,0,0,0.3)',
    ...
    const renderCalendar = () => {
        ...
        return (
            <div style={{ ...bentoPanelStyle, padding: '2rem' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'var(--primary)', opacity: 0.8, boxShadow: '0 2px 10px rgba(var(--primary-rgb), 0.2)' }}></div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{
                            ...
                            color: 'var(--primary)', boxShadow: '0 4px 15px rgba(var(--primary-rgb), 0.1)'
                        }}>
                            <CalendarIcon size={24} />
                        </div>
                        <div>
                            <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '900', color: 'var(--text-main)', letterSpacing: '-0.5px' }}>Team Availability</h2>
                            <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: '600', marginTop: '2px' }}>Monthly presence matrix · {currentMonth.format('MMMM YYYY')}</p>
                        </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: isLightMode ? '#f1f5f9' : 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '16px', border: `1px solid ${isLightMode ? '#e2e8f0' : 'rgba(255,255,255,0.06)'}` }}>
                        <button style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '6px', borderRadius: '10px', transition: 'all 0.2s' }} onMouseOver={e => e.currentTarget.style.background = 'rgba(0,0,0,0.05)'} onFocus={e => e.currentTarget.style.background = 'rgba(0,0,0,0.05)'} onMouseOut={e => e.currentTarget.style.background = 'transparent'} onClick={handlePrevMonth}><ChevronLeft size={18} /></button>
                        <span style={{ fontSize: '0.9rem', fontWeight: '800', minWidth: '130px', textAlign: 'center', color: 'var(--text-main)' }}>{currentMonth.format('MMMM YYYY')}</span>
                        <button style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '6px', borderRadius: '10px', transition: 'all 0.2s' }} onMouseOver={e => e.currentTarget.style.background = 'rgba(0,0,0,0.05)'} onFocus={e => e.currentTarget.style.background = 'rgba(0,0,0,0.05)'} onMouseOut={e => e.currentTarget.style.background = 'transparent'} onClick={handleNextMonth}><ChevronRight size={18} /></button>
                    </div>
                </div>
                                            ...
                                            >
                                                {status && (
                                                    <div style={{
                                                        ...
                                                        onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.8)'; e.currentTarget.style.boxShadow = `0 0 15px ${status.color}`; }}
                                                        onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = `0 0 10px ${status.color}70`; }}
                                                    >
                                                        {/* Inner glow dot */}
                                                        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '4px', height: '4px', borderRadius: '50%', background: 'white', opacity: 0.5 }}></div>
                                                    </div>
                                                )}
                                            </div>
                                        );
                ...
                {/* Legend */}
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', paddingTop: '1.25rem', borderTop: `1px solid ${isLightMode ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.05)'}` }}>
                    {[
                        { label: 'Present', color: statusColors.present, desc: 'At desk' },
                        { label: 'Leave', color: statusColors.leave, desc: 'Out' },
                        { label: 'Half Day', color: statusColors.halfDay, desc: 'Morning/Evening' },
                        ...
                        <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <span style={{ fontSize: '0.75rem', fontWeight: '900', color: 'var(--text-main)', lineHeight: 1 }}>{l.label}</span>
                                <span style={{ fontSize: '0.55rem', fontWeight: '700', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{l.desc}</span>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    return (
        <div style={{ padding: '0 1.5rem', marginTop: '1rem', position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                <div style={{ ...bentoPanelStyle, padding: '1.75rem' }}>
                    {/* Top Accent Bar */}
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#f43f5e', boxShadow: '0 2px 10px rgba(244, 63, 94, 0.3)' }}></div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            ...
                            <div>
                                <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '900', color: 'var(--text-main)', letterSpacing: '-0.3px' }}>Who is off today</h3>
                                <p style={{ margin: 0, fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: '600' }}>Missing from action</p>
                            </div>
                        </div>
                        {dashData.leaves?.length > 0 && <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#f43f5e', background: 'rgba(244, 63, 94, 0.1)', padding: '0.25rem 0.75rem', borderRadius: '20px' }}>{dashData.leaves.length} OFF</span>}
                    </div>
                    
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', minHeight: '60px', alignItems: 'center' }}>
                        {dashData.leaves && dashData.leaves.length > 0 ? dashData.leaves.map(l => (
                            <div key={l._id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '16px', background: 'linear-gradient(135deg, #f43f5e, #fb7185)', color: 'white', fontSize: '1.1rem', fontWeight: '900', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(244, 63, 94, 0.2)' }}>
                                    {l.user?.name?.substring(0, 2).toUpperCase()}
                                </div>
                                <span style={{ fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-main)', maxWidth: '65px', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.user?.name?.split(' ')[0]}</span>
                            </div>
                        )) : null}
                    </div>
                </div>
                ...
                <div style={{ ...bentoPanelStyle, padding: '1.75rem' }}>
                    {/* Top Accent Bar */}
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#f59e0b', boxShadow: '0 2px 10px rgba(245, 158, 11, 0.3)' }}></div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            ...
                            <div>
                                <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '900', color: 'var(--text-main)', letterSpacing: '-0.3px' }}>Not in yet today</h3>
                                <p style={{ margin: 0, fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: '600' }}>Awaiting check-in</p>
                            </div>
                        </div>
                        {dashData.notInYet?.length > 0 && <span style={{ fontSize: '0.7rem', fontWeight: '900', color: '#f59e0b', background: 'rgba(245, 158, 11, 0.1)', padding: '0.25rem 0.75rem', borderRadius: '20px' }}>{dashData.notInYet.length} NOT IN</span>}
                    </div>

                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', minHeight: '60px', alignItems: 'center' }}>
                        {dashData.notInYet && dashData.notInYet.length > 0 ? dashData.notInYet.map(m => (
                            <div key={m._id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '16px', background: 'linear-gradient(135deg, #f59e0b, #fbbf24)', color: 'white', fontSize: '1.1rem', fontWeight: '900', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(245, 158, 11, 0.2)' }}>
                                    {m.name.substring(0, 2).toUpperCase()}
                                </div>
                                <span style={{ fontSize: '0.7rem', fontWeight: '800', color: 'var(--text-main)', maxWidth: '65px', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.name.split(' ')[0]}</span>
                            </div>
                        )) : (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#22c55e', padding: '1rem' }}>
                                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(34, 197, 94, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✓</div>
                                <span style={{ fontSize: '0.85rem', fontWeight: '700' }}>Full House! All checked in.</span>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
                <div style={{ ...bentoPanelStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.75rem' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#22c55e', boxShadow: '0 2px 10px rgba(34, 197, 94, 0.3)' }}></div>
                    <div>
                        <div style={{ fontSize: '0.7rem', fontWeight: '900', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.75rem' }}>On Time Today</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <div style={{ fontSize: '2.5rem', fontWeight: '900', color: 'var(--text-main)', lineHeight: '1' }}>{stats.onTimeCount}</div>
                            <div style={{ height: '24px', width: '1px', background: 'rgba(0,0,0,0.1)' }}></div>
                            <span style={{ fontSize: '0.72rem', fontWeight: '900', color: 'var(--primary)', cursor: 'pointer', background: 'rgba(var(--primary-rgb), 0.1)', padding: '0.3rem 0.75rem', borderRadius: '20px' }} onClick={() => setShowEmployeesPanel(true)}>View All</span>
                        </div>
                    </div>
                    <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, rgba(34, 197, 94, 0.15), rgba(34, 197, 94, 0.05))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Clock size={32} color="#22c55e" />
                    </div>
                </div>

                <div style={{ ...bentoPanelStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.75rem' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#ef4444', boxShadow: '0 2px 10px rgba(239, 68, 68, 0.3)' }}></div>
                    <div>
                        <div style={{ fontSize: '0.7rem', fontWeight: '900', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.75rem' }}>Late Arrivals</div>
                        <div style={{ fontSize: '2.5rem', fontWeight: '900', color: 'var(--text-main)', lineHeight: '1' }}>{stats.lateCount}</div>
                    </div>
                    <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.15), rgba(239, 68, 68, 0.05))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Clock size={32} color="#ef4444" />
                    </div>
                </div>

                <div style={{ ...bentoPanelStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.75rem' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#a855f7', boxShadow: '0 2px 10px rgba(168, 85, 247, 0.3)' }}></div>
                    <div>
                        <div style={{ fontSize: '0.7rem', fontWeight: '900', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.75rem' }}>Remote Work</div>
                        <div style={{ fontSize: '2.5rem', fontWeight: '900', color: 'var(--text-main)', lineHeight: '1' }}>{stats.remoteCount}</div>
                    </div>
                    <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.15), rgba(168, 85, 247, 0.05))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Laptop size={32} color="#a855f7" />
                    </div>
                </div>
            </div>
            ...
            <div style={{ marginTop: '3rem', marginBottom: '3rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '900', color: 'var(--text-main)', letterSpacing: '-0.5px' }}>Your Peers</h3>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '2px' }}>
                            <span style={{ fontSize: '0.75rem', fontWeight: '700', color: 'var(--text-muted)' }}>Collaborate with your department</span>
                            <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--text-muted)', opacity: 0.3 }}></span>
                            <span style={{ fontSize: '0.75rem', fontWeight: '800', color: 'var(--primary)' }}>{teammates.length + 1} Members</span>
                        </div>
                    </div>
                </div>
                    ...
                    {[user, ...teammates].map(t => {
                        ...
                        return (
                            <div key={t._id} style={{ 
                                ...
                                gap: '1.25rem', 
                                cursor: 'pointer',
                                border: isUser ? `1.5px solid var(--primary)` : bentoPanelStyle.border,
                                background: isUser ? (isLightMode ? 'rgba(var(--primary-rgb), 0.05)' : 'rgba(var(--primary-rgb), 0.08)') : bentoPanelStyle.background,
                            }} 
                            onMouseOver={e => { 
                                e.currentTarget.style.transform = 'translateY(-5px)'; 
                                e.currentTarget.style.boxShadow = isLightMode ? '0 20px 40px rgba(0,0,0,0.08)' : '0 20px 50px rgba(0,0,0,0.4)'; 
                            }} 
                            onMouseOut={e => { 
                                e.currentTarget.style.transform = 'translateY(0)'; 
                                e.currentTarget.style.boxShadow = bentoPanelStyle.boxShadow; 
                            }}>
                                <div style={{
                                    width: '52px', height: '52px', borderRadius: '18px', fontSize: '1.2rem', fontWeight: '900', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
                                ...
                                <div style={{ flex: 1, overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                        <div style={{ fontWeight: '800', fontSize: '1rem', color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.name}</div>
                                        {isUser && <span style={{ fontSize: '0.6rem', fontWeight: '900', color: 'var(--primary)', background: 'rgba(var(--primary-rgb), 0.15)', padding: '2px 6px', borderRadius: '6px' }}>YOU</span>}
                                    </div>
                                    <div style={{ fontSize: '0.75rem', fontWeight: '600', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: '2px' }}>{t.designation}</div>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', alignItems: 'flex-end' }}>
                                    {isNotIn && <div style={{ fontSize: '0.6rem', padding: '0.3rem 0.6rem', background: 'rgba(245, 158, 11, 0.15)', color: '#f59e0b', borderRadius: '30px', fontWeight: '900', letterSpacing: '0.5px' }}>NOT IN</div>}
                                    {isOff && <div style={{ fontSize: '0.6rem', padding: '0.3rem 0.6rem', background: 'rgba(244, 63, 94, 0.15)', color: '#f43f5e', borderRadius: '30px', fontWeight: '900', letterSpacing: '0.5px' }}>OFF</div>}
                                    {!isNotIn && !isOff && <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 8px #22c55e' }}></div>}
                                </div>
                            </div>
                        );
                    })}
            ...
            {showEmployeesPanel && (
                <>
                    ...
                    <div style={{
                        ...
                        {/* Header */}
                        <div style={{ padding: '2.5rem 2rem', borderBottom: `1px solid ${isLightMode ? '#f1f5f9' : 'rgba(255,255,255,0.06)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(to right, transparent, rgba(var(--primary-rgb), 0.02))' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '16px', background: 'rgba(var(--primary-rgb), 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 15px rgba(var(--primary-rgb), 0.1)' }}>
                                    <Users size={22} color="var(--primary)" />
                                </div>
                                <div>
                                    <h2 style={{ fontSize: '1.4rem', margin: 0, fontWeight: '900', color: 'var(--text-main)', letterSpacing: '-0.6px' }}>Staff Directory</h2>
                                    <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: '600', marginTop: '0.2rem' }}>Detailed list and status filtering</p>
                                </div>
                            </div>
                            <button style={{ width: '40px', height: '40px', borderRadius: '12px', background: isLightMode ? '#f1f5f9' : 'rgba(255,255,255,0.06)', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }} onClick={() => setShowEmployeesPanel(false)} onMouseOver={e => { e.currentTarget.style.background = '#ef4444'; e.currentTarget.style.color = '#fff'; }} onMouseOut={e => { e.currentTarget.style.background = isLightMode ? '#f1f5f9' : 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = 'var(--text-muted)'; }}>
                                <X size={20} />
                            </button>
                        </div>
